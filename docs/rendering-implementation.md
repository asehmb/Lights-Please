...
4. During each frame, `Engine::render()` calls `RenderSystem::update(entityManager, sceneGraph, renderer)`.
5. `RenderSystem::update`:
   - queries ECS archetypes containing `Transformable | Renderable`
   - resolves `meshId/materialId` to pointers from its internal registries
   - resolves transform handles through `SceneGraph` and builds `Renderer::Drawable` world transforms from the authoritative transform graph
...
- `SceneGraph`: owns spatial transform data (local TRS + world matrices + hierarchy) addressed by `Transformable::handle`.
