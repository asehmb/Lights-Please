#pragma once

#include "math/vector.hpp"
#include <SDL_events.h>
#include <vulkan/vulkan.h>

class Camera {
public:
  Camera();
  ~Camera();
  mathplease::Vector3 position = mathplease::Vector3(4.0f, 4.0f, 4.0f);
  mathplease::Vector3 velocity = mathplease::Vector3(1.f, 1.0f, 1.0f);

  float pitch = -0.96f;
  float yaw = 0.96f; // Start looking at -Z
  float fov = 45.0f;
  float aspectRatio = 1.77778f; // Default to 16:9

  // Camera control settings
  float movementSpeed = 1.0f;
  float mouseSensitivity = 0.005f;
  float pitchConstraint = 89.0f; // Degrees
  bool invertY = false;

  // Input state
  struct InputState {
    bool forward = false;
    bool backward = false;
    bool left = false;
    bool right = false;
    bool up = false;
    bool down = false;
    bool sprint = false;
    bool mouseCapture = false;
  } input;

  mathplease::Matrix4 getViewMatrix();
  mathplease::Matrix4 getRotationMatrix();
  mathplease::Matrix4 getProjectionMatrix(float nearPlane = 0.1f,
                                          float farPlane = 1000.0f);

  /* will be removed */
  void update(float deltaTime);

  void handleEvent(const SDL_Event &event);

  // Camera utilities
  mathplease::Vector3 getForward();
  mathplease::Vector3 getRight();
  mathplease::Vector3 getUp();
};
