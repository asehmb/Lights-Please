...
Matrix4 Matrix4::perspective(float fovYRadians, float aspectRatio,
...
Vector4 Matrix4::operator*(const Vector4 &vec) const {
  return Vector4(m[0] * vec.x + m[4] * vec.y + m[8] * vec.z + m[12] * vec.w,
...
Vector3 Matrix4::transformPoint(const Vector3 &point) const {
  ...
  if (result.w != 0.0f) {
    return Vector3(result.x / result.w, result.y / result.w,
                   result.z / result.w);
  }
  return result.xyz();
}
...
float Matrix4::determinant() const {
  float det = m[0] * (m[5] * (m[10] * m[15] - m[11] * m[14]) -
