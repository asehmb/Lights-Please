...
mathplease::Matrix4 Camera::getViewMatrix() {
  ...
  mathplease::Matrix4 translationInv =
      mathplease::Matrix4::translate(-position);

  return rotationInv * translationInv;
}

mathplease::Matrix4 Camera::getProjectionMatrix(float nearPlane,
                                                float farPlane) {
  return mathplease::Matrix4::perspective(fov * (M_PI / 180.0f), aspectRatio,
                                          nearPlane, farPlane);
}
...
mathplease::Vector3 Camera::getForward() {
  mathplease::Matrix4 rotation = getRotationMatrix();

  // Camera looks down local -Z
  return rotation.transformVector(mathplease::Vector3(0.0f, 0.0f, -1.0f))
      .normalized();
}

mathplease::Vector3 Camera::getRight() {
  mathplease::Matrix4 rotation = getRotationMatrix();

  return rotation.transformVector(mathplease::Vector3(1.0f, 0.0f, 0.0f))
      .normalized();
}

mathplease::Vector3 Camera::getUp() {
  mathplease::Matrix4 rotation = getRotationMatrix();

  return rotation.transformVector(mathplease::Vector3(0.0f, 1.0f, 0.0f))
      .normalized();
}
